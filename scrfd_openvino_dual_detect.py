...
class SCRFD:
    def __init__(self, model_file=None, compiled_model=None):
    ...
    def autodetect(self, img, max_num=0, metric='max'):
        # Use model's fixed input dimensions for all detections
        # Try different thresholds instead of different input sizes
        bboxes, kpss = self.detect(img, input_size=self.input_size, thresh=0.5)
        bboxes2, kpss2 = self.detect(img, input_size=self.input_size, thresh=0.3)  # Lower threshold for second pass
        
        # Combine results from both threshold passes
        if bboxes.shape[0] > 0 and bboxes2.shape[0] > 0:
            ...
            bboxes_all = bboxes
            kpss_all = kpss
        elif bboxes2.shape[0] > 0:
            bboxes_all = bboxes2
            kpss_all = kpss2
        else:
            # No detections found
            return np.array([]).reshape(0, 5), np.array([]).reshape(0, 5, 2) if self.use_kps else None
        
        # Apply NMS to combined results
        keep = self.nms(bboxes_all)
        det = bboxes_all[keep, :]
        if self.use_kps and kpss_all is not None:
            kpss = kpss_all[keep, :]
        else:
            kpss = None
