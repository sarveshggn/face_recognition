...
import cv2
import numpy as np
from fr.adaface_onnx import AdaFaceONNX
from fr.adaface_openvino import AdaFaceOpenVINO
from fd.scrfd import SCRFD
...
adaface_model_path = os.path.join(assets_dir, './Adaface/R50/I8/adaface_r50_int8.xml')
...
adaface_path = "Embeddings/Adaface_ov_int8"
...
def go_through_adaface():
    ...
    for photo_dir in photo_dirs:
        ...
        for image in images:
            ...
            c.execute('''SELECT adaface_embedding_ov_int8 FROM photos WHERE path = ?''', (img_path,))
            result = c.fetchone()
            ...
            c.execute('''UPDATE photos SET adaface_embedding_ov_int8 = ? WHERE path = ?''',
